...
function loadAssets(callback) {
  ...
  backgroundImage.onload = function () {
    ...
    characterImage1.onload = function () {
      // Load the second character image
      ...
      characterImage2.onload = function () {
        ...
        floorImage.onload = function () {
          ...
          explosionImage.onload = function () {
            // Load the attack image (CORRECTED CODE)
            attackImage.src = 'attack.png'; // Replace 'attack.png' with the path to your attack image
            attackImage.onload = function () {
              // Load the enemy image (CORRECTED CODE)
              enemyImage.src = 'enemy.png'; // Replace 'enemy.png' with the path to your enemy image
              enemyImage.onload = function () {
                generateEnemies(); // Call the function to generate enemies after loading the images
                callback();
              };
            };
          };
...
class Enemy {
  constructor(x, y, width, height, velocityX, imageSrc) {
    this.x = x;
    this.y = y;
    this.width = width;
    this.height = height;
...
enemyImage.onload = function() {
  // The image is loaded, now you can call the generateEnemies() function
  generateEnemies();
};
function generateEnemies() {
  platforms.forEach((platform) => {
    const platformMaxY = platform.y - player.jumpHeight * 2;
    const platformMinY = platform.y + platform.height;
    const platformInView = platform.x + platform.width > camera.x && platform.x < camera.x + camera.width;
if (!platform.enemiesGenerated && platformInView) {
      const numberOfEnemies = Math.min(Math.floor(Math.random() * 3) + 1, 2);
      const enemySpacing = platform.width / (numberOfEnemies + 1);

      for (let i = 0; i < numberOfEnemies; i++) {
        const enemyX = platform.x + enemySpacing * (i + 1);
        const enemyY = platform.y - 50;
        const enemyWidth = 30;
        const enemyHeight = 30;
        const enemyVelocityX = Math.random() < 0.5 ? -2 : 2;
        enemies.push(new Enemy(enemyX, enemyY, enemyWidth, enemyHeight, enemyVelocityX, enemyImage.src));
      }
      platform.enemiesGenerated = true;
    }
  });
}
...
function detectPlatformCollision() {
    ...
    function detectEnemyCollision() {
      if (player.isAttacking) {
        enemies.forEach((enemy, index) => {
          // Check for collision between the character and the enemy during an attack
          if (
            ...
            // Enemy dies instantly on attack
            enemies.splice(index, 1);
            player.score++; // Increase score on successful attack
          }
        });
      }
    }
  ...
  function update() {
    if (player.isAttacking) {
      ...
      // Check if the attack animation has reached its end (adjust the value as needed)
      if (player.explosionCounter >= 25) {
    ...
    if (!gameOver) {
      ...
      // Generate new platforms and enemies as the player moves
      generatePlatforms();
    }
  ...
  function drawCharacter() {
    ...
    if (!gameOver) {
      ...
      if (player.isAttacking) {
        ...
        // Draw the regular character image
        ctx.drawImage(
          player.currentImage,
          player.x,
          player.y,
